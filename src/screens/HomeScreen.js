...
export default function HomeScreen({ log, upLog, data, last5, fmtT, totalSt, studyPct, T, streak }) {
  ...
  return (
    <ScrollView style={{ flex:1, backgroundColor:T.bg }} contentContainerStyle={{ padding:18, paddingBottom:30 }} showsVerticalScrollIndicator={false}>

      {/* Banner */}
      <View style={{ backgroundColor:T.aDim, borderWidth:1, borderColor:T.aBd,
        borderRadius:20, padding:20, marginBottom:14 }}>
        <Text style={{ fontSize:10, fontWeight:'700', color:T.accent, letterSpacing:1.2, textTransform:'uppercase', marginBottom:6 }}>{greet}</Text>
        <View style={{ flexDirection:'row', alignItems:'baseline', gap:4 }}>
          <Text style={{ fontSize:28, fontWeight:'800', color:T.accent, letterSpacing:-0.5 }}>DJ'</Text>
          <Text style={{ fontSize:28, fontWeight:'800', color:T.text, letterSpacing:-0.5 }}>Tracker</Text>
        </View>
        <Text style={{ fontSize:12, color:T.sub, marginTop:4 }}>Dhanush Jaddu</Text>
      ...
      <View style={{
        ...
        backgroundColor:T.card, borderWidth:1, borderColor: streak > 0 ? `${T.amber}30` : T.glassBd,
        borderRadius:16, padding:16, marginBottom:14,
      }}>
        <View style={{ flexDirection:'row', alignItems:'center', gap:12 }}>
          <View style={{
            width:44, height:44, borderRadius:22,
            backgroundColor: streak > 0 ? `${T.amber}18` : T.cardHi,
            borderWidth:1, borderColor: streak > 0 ? `${T.amber}40` : T.border,
            alignItems:'center', justifyContent:'center',
          }}>
            <Text style={{ fontSize:20 }}>{streak > 0 ? '🔥' : '💤'}</Text>
          </View>
          <View>
            <Text style={{ fontSize:11, fontWeight:'700', color:T.sub, letterSpacing:0.8, textTransform:'uppercase' }}>
              {streak > 0 ? 'Current Streak' : 'No Streak'}
            </Text>
            <Text style={{ fontSize:24, fontWeight:'800', color: streak > 0 ? T.amber : T.dim, letterSpacing:-0.5 }}>
              {streak} {streak === 1 ? 'day' : 'days'}
            </Text>
          </View>
        </View>
        <View style={{ alignItems:'center' }}>
          <Text style={{ fontSize:9, color:T.dim, letterSpacing:0.5 }}>
      ...
      <Card T={T} style={{ marginBottom:14 }}>
        ...
        <Field T={T} value={String(log.steps||'')} onCommit={v=>upLog({steps:v})}
          ...
          style={{ textAlign:'center', fontWeight:'700', fontSize:16, marginTop:14 }}/>
      </Card>
      ...
      <View style={{ flexDirection:'row', flexWrap:'wrap', gap:10, marginBottom:14 }}>
        {[
          ...
          <View key={i} style={{ width:'48%', backgroundColor:T.card, borderWidth:1, borderColor:T.glassBd, borderRadius:16, padding:14 }}>
            <Text style={{ fontSize:10, fontWeight:'700', color:T.sub, letterSpacing:0.8, textTransform:'uppercase', marginBottom:10 }}>{s.l}</Text>
            <View style={{ flexDirection:'row', justifyContent:'space-between', alignItems:'flex-end' }}>
              <View>
                <Text style={{ fontSize:22, fontWeight:'800', color:T.text, letterSpacing:-0.5 }}>{s.v}</Text>
                <Text style={{ fontSize:10, color:T.dim, marginTop:4 }}>{s.s}</Text>
              </View>
      ...
      {/* Charts */}
      <Card T={T} style={{ marginBottom:14 }}>
        <Lbl T={T} right={`avg ${(studyD5.reduce((a,d)=>a+d.v,0)/5).toFixed(1)}h`}>Study — 5 Days</Lbl>
        <Bar5 T={T} d5={studyD5} maxV={data.profile.goal_study} col={T.accent} unit="h"/>
      </Card>
      <Card T={T} style={{ marginBottom:14 }}>
        <Lbl T={T}>Steps — 5 Days</Lbl>
        <Bar5 T={T} d5={stepD5} maxV={data.profile.goal_steps} col={T.green}/>
      </Card>
      ...
      <View style={{ flexDirection:'row', gap:10, marginBottom:14 }}>
        <View style={{ flex:3, backgroundColor:T.card, borderWidth:1, borderColor:T.glassBd, borderRadius:16, padding:14 }}>
          <Lbl T={T}>Mood</Lbl>
          <View style={{ flexDirection:'row', justifyContent:'space-between' }}>
            {MOODS.map((m,i) => (
              <TouchableOpacity key={i} onPress={() => upLog({mood:m.e})} style={{ alignItems:'center', gap:4 }}>
        ...
        <View style={{ flex:2, backgroundColor:T.card, borderWidth:1, borderColor:T.glassBd, borderRadius:16, padding:14 }}>
      ...
      <Card T={T} accent style={{ marginBottom:14 }}>
        <Text style={{ fontSize:10, fontWeight:'700', color:T.sub, letterSpacing:1.4, textTransform:'uppercase', textAlign:'center', marginBottom:18 }}>Focus Timer</Text>
        ...
        <Field T={T} value={subject} onCommit={() => {}}
          ...
          placeholder="Subject / Topic" style={{ textAlign:'center', fontSize:14, marginTop:20, marginBottom:16 }}/>
      ...
      <Card T={T} style={{ marginBottom:14 }}>
        ...
        <Text style={{ fontSize:11, color:T.dim, marginTop:8 }}>{(totalSt/3600).toFixed(2)}h studied today</Text>
      </Card>
      ...
      <Card T={T}>
        ...
        {(log.sessions||[]).length===0
          ? <Text style={{ fontSize:13, color:T.dim, paddingVertical:10 }}>No sessions yet — start the timer above.</Text>
          : (log.sessions||[]).map((s,i) => (
            <View key={i} style={{ flexDirection:'row', justifyContent:'space-between', alignItems:'center',
              paddingVertical:12, borderBottomWidth:i<(log.sessions||[]).length-1?1:0, borderBottomColor:T.border }}>
              <View>
                <Text style={{ fontSize:13, fontWeight:'600', color:T.text }}>{s.s}</Text>
                <Text style={{ fontSize:11, color:T.dim, marginTop:3 }}>{s.at}</Text>
              </View>
